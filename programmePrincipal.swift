...
while (!p.finDePartie()) {
	...
	p.afficherPartie() //affiche le plateau avec les pions des 2 joueurs.0
 	...
 	if(existeDeplacement(p.joueurCourant)){ //vérifie si un déplacement est possible avec les cartes qu'il possède
		...
		repeat {
			...
			var pionChoisi: TPion = getPion(joueur : p.joueurCourant, position : positionPionChoisi) //récupérer le pion choisi
