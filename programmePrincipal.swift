...
while (!p.finDePartie()) {
	print(p.afficherPartie()) //affiche le plateau avec les pions des 2 joueurs sous forme textuelle
 	...
 	if(existeDeplacement(p.joueurCourant)){ //vérifie si un déplacement est possible avec les cartes qu'il possède
		...
		repeat {
			...
			print(p.joueurCourant.afficherCartes() )
			print(p.joueurCourant.afficherPionsEnVie() )

		    	//Le joueur choisis un de ses pions toujours en vie
			repeat {
				print("Il faut choisir la position d'un des pions parmis ceux proposés : \n")
				var x: Int = readLine()
				var y: Int = readLine() //Le joueur saisi en console la position du pion qu'il veut bouger
			} while !(p.joueurCourant.existePion(x: x, y: y)) //redemande un pion tant qu'il ne saisie pas la position d'un de ses pions en vie
			var pionChoisi: TPion = p.joueurCourant.getPion(position: positionPionChoisi) //récupérer le pion choisi

		    	//Le joueur choisis maintenant une carte 
			repeat {
				print("Il faut saisir le nom d'une des cartes parmis celles proposées : \n")
				var nomCarteChoisie: String = readLine() //Le joueur saisi en console le nom de la carte qu'il veut jouer
			...
			var carteChoisie: TCarte = p.joueurCourant.getCarte(nom: nomCarteChoisie) //récupérer la carte choisie
			...
			repeat {
				...
				var x: Int = readLine()
				var y: Int = readLine()
			} while !( carteChoisie.deplacementAppartientMotif(x: x, y: y) ) //vérifier que le déplacement appartient à la carte que le joueur à choisi
            			
			//vérifier que le mouvement que le mouvement choisit est possible avec sa carte et son pion qu'il a choisi
		    	if (pionChoisi.peutBouger(x: x, y: y)){ 
				p.joueurCourant.bougerPion(pion: pionChoisi, x: x, y: y) //je tue le pion adverse si besoin lorsque je bouge
		...
		repeat {
			...
			var nomCarteChoisie: String = readLine() //Le joueur saisi en console le nom de la carte qu'il veut 	
		} while !p.joueurCourant.existeCarte(nom: nomCarteChoisie) //redemande une carte tant qu'il ne saisie pas le nom d'une de ses cartes
		var carteChoisie: TCarte = p.joueurCourant.getCarte(nom: nomCarteChoisie ) //récupérer la carte choisie 
